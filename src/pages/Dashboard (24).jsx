...
export default function Dashboard() {
  ...
  const [mapaExpandido, setMapaExpandido] = useState(false) // controla o modo expandido (tela cheia) do mapa
  ...
  // Alterna o mapa entre o modo normal (dentro do card) e o modo expandido (tela cheia)
  const toggleMapaExpandido = () => setMapaExpandido(prev => !prev)

  // Fecha o mapa expandido com a tecla Esc, por conveniência
  useEffect(() => {
    if (!mapaExpandido) return
    const handleKeyDown = (e) => {
      if (e.key === 'Escape') setMapaExpandido(false)
    }
    document.addEventListener('keydown', handleKeyDown)
    return () => document.removeEventListener('keydown', handleKeyDown)
  }, [mapaExpandido])

  // O Leaflet calcula o tamanho do mapa no momento em que é montado; ao mudar o container de
  // tamanho (expandir/reduzir) é preciso "cutucar" o mapa para ele recalcular as dimensões.
  useEffect(() => {
  ...
  return (
    <Layout title="Dashboard" subtitle="Visão geral do sistema de gestão">
      ...
      {/* Fundo escurecido atrás do mapa quando ele está expandido */}
      {mapaExpandido && (
        <div
      ...
      {/* ── Mapa de Imóveis (esquerda) + Seguro Fiança / Seguro Incêndio / Ocupações / Lucro (direita) ── */}
      <div className="mb-4 grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_360px] xl:items-start">
        <Card
          className={
            mapaExpandido
              ? 'fixed inset-3 z-50 flex flex-col overflow-hidden sm:inset-6'
              : ''
          }
        >
          <CardHeader className="flex w-full flex-col flex-wrap gap-2 border-b py-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-2">
              ...
              >
                {mapaExpandido ? <Minimize2 className="size-3.5" /> : <Maximize2 className="size-3.5" />}
              </Button>
            </div>
          </CardHeader>
          <CardContent className={mapaExpandido ? 'min-h-0 flex-1 p-3' : 'p-3'}>
            <div className={mapaExpandido ? 'h-full' : 'h-[420px]'}>
        ...
        <div className="flex flex-col gap-3">
          {segurosExpirandoFianca.length > 0 && (
            <Card className="border-amber-300" style={{ background: '#fffbeb' }}>
          ...
          {segurosExpirandoIncendio.length > 0 && (
            <Card className="border-orange-300" style={{ background: '#fff7ed' }}>
              <CardHeader className="">
                    ...
                    {lucroPorMes.map((m, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`text-[9px] ${isMax ? 'font-semibold text-emerald-700' : 'text-muted-foreground'}`}>
                            {MONTH_LABELS[index]}
                          </span>
                        </div>
                      )
                  ...
                  <div key={item.id} className="group flex items-center justify-between gap-3 py-2 text-xs first:pt-0 last:pb-0">
                    <div className="flex min-w-0 items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        ...
                        {/* Valor do débito e mês de referência no momento da alteração */}
                        <p className="truncate text-muted-foreground">
                          {item.valorDebito != null ? fmtMoney(item.valorDebito) : '—'}
                          {item.mesReferencia ? ` · ${formatMonthKeyShort(item.mesReferencia)}` : ''}
                        </p>
