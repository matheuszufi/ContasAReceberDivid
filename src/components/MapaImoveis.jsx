...
// Estilo do botão de expandir/recolher, reaproveitado nos dois estados
...
// possui coordenadas (im.geo.lat / im.geo.lng). Pode ser expandido para tela cheia através
// do botão no canto superior direito (ou tecla Esc / clique fora para recolher).
export function MapaImoveis({ imoveis }) {
  ...
  // Inicializa o mapa uma única vez. O container (mapContainerRef) nunca é desmontado — apenas
  // seu tamanho/posição mudam via CSS quando o mapa é expandido — então o mapa do Leaflet
  // permanece o mesmo durante toda a vida do componente.
  useEffect(() => {
  ...
  // Bloqueia o scroll da página enquanto o mapa está expandido em tela cheia
  useEffect(() => {
  ...
  // Permite recolher o mapa expandido pressionando Esc
  useEffect(() => {
    if (!isExpanded) return undefined
    const handleKeyDown = (e) => { if (e.key === 'Escape') setIsExpanded(false) }
    window.addEventListener('keydown', handleKeyDown)
    return () => window.removeEventListener('keydown', handleKeyDown)
  }, [isExpanded])

  // Reajusta o tamanho do mapa (e reenquadra os imóveis) sempre que ele é expandido/recolhido,
  // já que o Leaflet não detecta automaticamente mudanças de tamanho feitas via CSS
  useEffect(() => {
    ...
    <>
      {isExpanded && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.6)', zIndex: 1999 }}
        />
      )}
      <div
        style={
